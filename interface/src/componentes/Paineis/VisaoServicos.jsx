...
export default function VisaoServicos({ empresaId }) {
  ...
  const [form, setForm] = useState({ nome: '', descricao: '', preco: '', duracao: 30 });
  ...
  const abrirModal = (item = null) => {
    if (item) {
      ...
      setEditando(null);
      setForm({ nome: '', descricao: '', preco: '', duracao: 30 });
    }
    setModalAberto(true);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-800">Catálogo de Serviços</h1>
          <p className="text-slate-500 font-medium">Gerencie seus serviços principais e adicionais.</p>
        </div>
        <button 
          onClick={() => abrirModal()}
        ...
        >
          <Plus size={20}/> Novo {abaAtiva === 'principais' ? 'Serviço' : 'Adicional'}
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-2 rounded-[2rem] border border-slate-200 shadow-sm">
        <div className="flex p-1 bg-slate-100 rounded-2xl w-full md:w-auto">
          ...
          >
            <Briefcase size={18} /> Serviços Principais
          </button>
          <button 
      ...
      {carregando ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-[2rem] border-2 border-dashed border-slate-200 text-center px-6">
          ...
          <p className="text-slate-500 max-w-xs mb-8">Nenhum {abaAtiva === 'principais' ? 'serviço' : 'adicional'} encontrado para sua busca ou cadastro.</p>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden border border-slate-200">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                <h2 className="font-black text-slate-800 text-2xl">{editando ? 'Editar' : 'Novo'} {abaAtiva === 'principais' ? 'Serviço' : 'Adicional'}</h2>
